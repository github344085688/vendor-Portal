<template>
    <section id="mobile-first bg-indigo  ">
        <div class=" mt-16 mb-12 border-b border-slate-100 xl:mb-0 demo-sm:block dark:border-slate-800 h-20 overflow-x-hidden overflow-y-visible">
            <div class="mb-[-3px] flex max-w-7xl mx-auto px-6 sm:px-8 md:px-10">
                <div class="flex items-start flex-none ml-[40rem] w-32">
                    <div class="flex flex-col items-center ml-[-2px]">
                        <div class="w-px h-14  "  :class="[dragSimulationWindow > 1 ? 'bg-indigo-600 ml-[-2px]' :'bg-slate-100 dark:bg-slate-800']"></div>
                        <div class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1  "
                             :class="[dragSimulationWindow > 1 ? 'bg-indigo-600 ml-[-2px] ring-indigo-600' :'bg-slate-100 dark:bg-slate-800']"
                            ></div>
                    </div>
                    <div class="ml-1.5 rounded font-mono text-[0.625rem] leading-6 px-1.5 ring-1 ring-inset dark:ring-0   dark:bg-indigo-500 dark:text-white dark:highlight-white/10"
                         :class="[dragSimulationWindow > 1 ? 'bg-indigo-50 text-indigo-600 ring-indigo-600' :'bg-slate-100 ring-slate-100']">
                        sm
                    </div>
                </div>
                <div class="flex items-start flex-none w-64">
                    <div class="flex flex-col items-center ml-[-2px]">
                        <div class="w-px h-14 " :class="[dragSimulationWindow > 2 ? 'bg-indigo-600 ml-[-2px]' :'bg-slate-100 dark:bg-slate-800']"></div>
                        <div class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1  " :class="[dragSimulationWindow > 2 ? 'bg-indigo-600 ml-[-2px] ring-indigo-600' :'bg-slate-100 dark:bg-slate-800']"></div>
                    </div>
                    <div class="ml-1.5 rounded font-mono text-[0.625rem] leading-6 px-1.5 ring-1 ring-inset dark:ring-0  dark:bg-indigo-500 dark:text-white dark:highlight-white/10"
                         :class="[dragSimulationWindow > 2 ? 'bg-indigo-50 text-indigo-600 ring-indigo-600' :'bg-slate-100 ring-slate-100']">
                        md
                    </div>
                </div>
                <div class="flex items-start flex-none w-64">
                    <div class="flex flex-col items-center ml-[-2px]" >
                        <div class="w-px h-14" :class="[dragSimulationWindow > 3 ? 'bg-indigo-600 ml-[-2px]' :'bg-slate-100 dark:bg-slate-800']"></div>
                        <div class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1  " :class="[dragSimulationWindow > 3 ? 'bg-indigo-600 ml-[-2px] ring-indigo-600' :'bg-slate-100 dark:bg-slate-800']"></div>
                    </div>
                    <div class="ml-1.5 rounded font-mono text-[0.625rem] leading-6 px-1.5 ring-1 ring-inset dark:ring-0  dark:bg-indigo-500 dark:text-white dark:highlight-white/10"
                         :class="[dragSimulationWindow > 3 ? 'bg-indigo-50 text-indigo-600 ring-indigo-600' :'bg-slate-100 ring-slate-100']">
                        lg
                    </div>
                </div>
                <div class="flex items-start flex-none w-64">
                    <div class="flex flex-col items-center ml-[-2px]">
                        <div class="w-px h-14 bg-slate-100 dark:bg-slate-800"></div>
                        <div class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1 bg-white ring-slate-500/[0.15] dark:bg-slate-900 dark:ring-slate-700"></div>
                    </div>
                    <div class="ml-1.5 rounded font-mono text-[0.625rem] leading-6 px-1.5 ring-1 ring-inset dark:ring-0 bg-slate-100 ring-slate-100 dark:bg-slate-800 dark:highlight-white/5">
                        xl
                    </div>
                </div>
        ...
        <div class="relative pt-10 xl:pt-0 mt-10 demo-sm:-mt-2.5" style="top:-50px;">
            <div class="absolute top-0  inset-x-0 h-[37.5rem] bg-grid-slate-900/[0.04] bg-top [mask-image:linear-gradient(0deg,transparent,black)] dark:bg-grid-slate-100/[0.03] dark:bg-[center_top_-1px] dark:border-t dark:border-slate-100/5 top-0 "></div>
                ...
                <div class="relative top-0  h-full " >
                    <div class="max-w-7xl mx-auto sm:px-6 md:px-8  h-full "  >
                        <div class="relative h-full w-full" >
                            <div class="w-full absolute inset-x-0 h-full " style=" width: calc(100% + 20px) ">
                                <input
                                        class="tick-slider-input drag-input"
                                        type="range"
                                        min="0"
                                        v-model="value"
                                        max="100"
                        ...
                        <div class="shadow-xl sm:rounded-xl   demo-sm:min-w-0 demo-sm:max-w-none  h-full  "
                             :style="{ width: width + 'px'  }"
                        >
                            <div class="sm:rounded-xl ring-1 ring-slate-900/5">
                                <div class="sm:rounded-t-xl bg-gradient-to-b from-white to-[#FBFBFB] dark:bg-none dark:bg-slate-700 dark:highlight-white/10">
                                    <div class="py-2.5 grid items-center px-4 gap-8"
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <svg width="24" height="24" fill="none"
                                                ...
                                                <path d="m15 7-5 5 5 5" stroke="currentColor" stroke-width="2"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                            </svg>
                                            <svg width="24" height="24" fill="none"
                                                 class="ml-2 flex-none text-slate-400 dark:text-slate-500">
                                                <path d="m10 7 5 5-5 5" stroke="currentColor" stroke-width="2"
                                                      stroke-linecap="round" stroke-linejoin="round"></path>
                                            </svg>
                                        </div>
                                        <div>
                                    ...
                                    <div class="grid grid-cols-3 text-xs leading-5 overflow-hidden">
                                        <div class="pointer-events-none select-none bg-slate-100 text-slate-400 rounded-tr border border-slate-900/5 px-4 py-1.5 -mb-px -ml-px flex items-center justify-center space-x-2 dark:bg-slate-800 dark:text-slate-500">
                                            <svg width="17" height="10" fill="currentColor"
                                                 class="flex-none text-slate-300 dark:text-slate-500">
                                            ...
                                            <div class="truncate">UNIS-UI - Official Tailwind CSS Components
                                            </div>
                                        </div>
                                        <div class="pointer-events-none select-none text-slate-900 font-medium px-4 py-1.5 flex items-center justify-center space-x-2 dark:text-slate-200">
                                            <svg width="15" height="14" fill="currentColor"
                                                 class="flex-none text-indigo-600 dark:text-slate-400">
                                        ...
                                        <div class="pointer-events-none select-none bg-slate-100 text-slate-400 rounded-tl border border-slate-900/5 pl-4 pr-8 py-1.5 -mb-px -mr-4 flex items-center justify-center space-x-2 dark:bg-slate-800 dark:text-slate-500">
                                            ...
                                            <div class="truncate">UNIS-UI UI – Unstyled, fully accessible UI
                                                components
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="relative bg-white border-t border-slate-200 rounded-b-xl -mb-8 dark:bg-slate-800 dark:border-slate-900/50">
                                    <slot></slot>
