...
export default class SignInPage extends BaseVue {
    ...
    password: (value: any) => {
      if (value && value.length) {
        ...
        if (regex.test(value)) return true;
          ...
          return "At least 8 characters,Contain a number, Uppercase letter ,Lowercase letter";
      }
    ...
    LoginServers.getLocationHrefSpId(demo).subscribe(
      (res: any) => {
        if (res) {
          if (name == "google") {
            if (res.data)
              window.location.href = `${url}${res.data.google_sp_entity_id}`;
          }
          if (name == "facebook") {
            if (res.data)
              window.location.href = `${url}${res.data.facebook_sp_entity_id}`;
          }
        }
      },
      (err) => {
  ...
  public getRegisterLoging(name: string) {
    ...
    if (
      host === "localhost" ||
      host === "shipdev.unisco.com" ||
      host === "shipstage.unisco.com"
    ) {
      this.locationHrefSpId(
        0,
        "https://stagesso.unisco.com/samlsso?spEntityID=",
        name
      );
    }
  }
